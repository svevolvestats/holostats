...
// holo/venue-global.js
async function onRequest(context) {
  const url = new URL(context.request.url);
  const origin = url.origin;
  try {
    return await serveWithOG(context, {
      title: "Global Store Search | \u30DB\u30ED\u30AB\u7D71\u8A08\u5C40",
      description: "Search overseas hololive OFFICIAL CARD GAME stores \u2014 upcoming events, distance sort",
      pageUrl: `${origin}/venue-global`
    });
  } catch {
